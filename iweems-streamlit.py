...
st.set_page_config(
    page_title="iWEEMS",  # Title that appears on the browser tab
...
The models themselves are available via [Zenodo](https://doi.org/10.5281/zenodo.14626412), where we also offer a more detailed description. For comparison, we also offer vector models trained on two other historical latin corpora (LASLA and OperaMaiora) available from [https://embeddings.lila-erc.eu](https://embeddings.lila-erc.eu/#topnav).

This app emerged as an output of the [TOME project](https://tome.flu.cas.cz).
""")
...
def plot_gen(subcorpus, target, topn, coordinates3d_dict, filtered_vocab_df, vectors_dict):
    ...
    fig_sim = plot_similarity_matrix(similarity_df, title="Pairwise Vector Similarity")
    return fig, pd.DataFrame(nns_tuples, columns=["word", "similarity"]), fig_sim

# Show the plot
try:
    fig, df, fig_sim = plot_gen(subcorpus, target, topn, coordinates3d_dict, filtered_vocab_df, vectors_dict)
    st.plotly_chart(fig)
    st.dataframe(df)
    st.pyplot(fig_sim)
except:
